...
void getshape(Matrix3d mid1) {
  cout << "EDGE Matrix Shape:(" << mid1.dep << "," << mid1.wid << ","
       << mid1.high << ")" << endl;
}
// void getshape4d(Matrix4d m) {
//     cout << "EDGE Matrix Shape: (" << m.batch << "," << m.dep << "," << m.wid
//     << "," << m.high << ")" << endl;
...
Matrix4dDimensions getshape4d(Matrix4d m) {
  std::cout << "EDGE Matrix Shape: (" << m.batch << "," << m.dep << "," << m.wid
            << "," << m.high << ")" << std::endl;
  return {m.batch, m.dep, m.wid, m.high};
}
...
Matrix4d fetch_skipOne(Matrix3d m) {
  /**
   * This function takes a 3D matrix input m, skips every other row and column
   * in each matrix slice, and outputs a 4D matrix containing the skipped values
   * in separate slices.
   *
   * @param m - The input 3D matrix
   * @return A 4D matrix where each slice contains the skipped values from the
   * corresponding slice in m
   */
  ...
  3. Loop through each slice (depth) in m
     ...
     3b. Loop through every other row and column in this slice, copying the
  skipped values into the 2D matrix 3c. Add the 2D matrix as a slice in the
  corresponding output 3D matrix
  ...
  */
  ...
  Matrix4d output_total =
      CreateMatrix4d(4, new_depth, new_width / 2, new_height / 2);
